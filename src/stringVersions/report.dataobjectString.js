...
return function () {
  return {
    ...
    incrementPage: function () {
      if (this.currentPage == this.getNumberOfPages()) return;
      this.currentRow = null;
      this.currentPage++;
    },
    ...
    getSelectedLine: function () {
      ...
      for (let line of this.table.table) {
        if (
          line.page_number == this.currentPage &&
    ...
    getLineSetById: function (selectedLine) {
      ...
      if (!selectedLine.element_id) return [selectedLine];
    ...
    insertBlankLineAfter: function () {
      ...
      this.insertBlankLineAtPosition(
        this.table.table.indexOf(selectedLine) + 1,
        defaultId
      );
    },
    ...
    insertBlankLineBefore: function () {
      ...
      this.insertBlankLineAtPosition(
        this.table.table.indexOf(selectedLine),
        defaultId
      );
      if (
        (this.currentRow < this.MAX_ROWS_FIRST_PAGE && this.currentPage == 1) ||
    ...
    shiftSelectedLineUp: function () {
      ...
      if (lineSet.length < 1) return;

      let start = this.table.table.indexOf(lineSet[0]);
      let end =
        lineSet.length == 1
          ? start
          : this.table.table.indexOf(lineSet[lineSet.length - 1]);
      if (start == 0) return;
    ...
    getSizeOfUpperElement: function (position) {
      ...
      if (upperElementId == "" || upperElementId == undefined) return 1;

      let lineCounter = 0;
      while (
        position >= 0 &&
        this.table.table[position--].element_id == upperElementId
      )
        lineCounter++;
    ...
    shiftSelectedLineDown: function () {
      ...
      if (end >= this.table.table.length - 1) return;
    ...
    getSizeOfBottomElement: function (position) {
      ...
      if (bottomElementId == "" || bottomElementId == undefined) return 1;

      let lineCounter = 0;
      while (
        position < this.table.table.length &&
        this.table.table[position++].element_id == bottomElementId
      )
        lineCounter++;
    ...
    updateTableCounters: function () {
      ...
      for (let line of this.table.table) {
        ...
        if (
          (rowCounter == this.MAX_ROWS_FIRST_PAGE && pageCounter == 1) ||
    ...
    transformSelectedRowGlobalPositionToLocal: function () {
      ...
      let row =
        deltaRows >= 0
          ...
          : this.MAX_ROWS_FIRST_PAGE +
            (deltaRows % this.MAX_ROWS_ALL_PAGES) +
            1;
      this.currentRow = row;
    ...
    isSingleElementOnBottomInsert: function () {
      ...
      if (position >= this.table.table.length - 1) return false;
